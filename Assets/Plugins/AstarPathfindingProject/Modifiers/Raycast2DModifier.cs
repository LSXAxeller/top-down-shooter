...
namespace Pathfinding {
	 ...
	 * Either graph raycasts or Physics2D.Raycast */
	[AddComponentMenu ("Pathfinding/Modifiers/Raycast 2D Simplifier")]
	[System.Serializable]
	public class Raycast2DModifier : MonoModifier {

	#if UNITY_EDITOR
		[UnityEditor.MenuItem ("CONTEXT/Seeker/Add Raycast Simplifier Modifier")]
		public static void AddComp (UnityEditor.MenuCommand command) {
			(command.context as Component).gameObject.AddComponent (typeof(Raycast2DModifier));
		}
	#endif
		...
		public bool ValidateLine (GraphNode n1, GraphNode n2, Vector3 v1, Vector3 v2) {
			...
			if (useRaycasting) {
				...
				if (thickRaycast && thickRaycastRadius > 0) {
                    RaycastHit2D hit = Physics2D.CircleCast(v1 + raycastOffset, thickRaycastRadius, v2 - v1, (v2 - v1).magnitude, mask);
                    if(hit.collider != null)
                    {
						return false;
					}
				} else {
                    RaycastHit2D hit = Physics2D.Linecast(v1 + raycastOffset, v2 + raycastOffset, mask);
                    if(hit.collider != null)
                    {
						return false;
					}
